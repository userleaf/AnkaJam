...
while True:
    ...
    # ==========================================
    # ---> LEAD MODE TILT PITCH BEND (WITH DIAGNOSTICS) <---
    # ==========================================
    if not hasattr(hw, 'last_print_time'): hw.last_print_time = now
    do_print = (now - hw.last_print_time) > 0.5 # Only print twice a second

    try:
        raw_tilt_y = hw.read_tilt_y()
    except AttributeError:
        raw_tilt_y = 0.0

    if current_mode == "LEAD" and system_state not in ["MENU", "HEROJAM", "EARTRAINER"]:
        if abs(raw_tilt_y) < 1.5:
            target_pb = 8192
            if do_print: 
                print(f"[TILT] LEAD Mode | Tilt: {raw_tilt_y:.2f} (DEADZONE) | PB: {target_pb}")
        else:
            mapped_val = int(((raw_tilt_y + 9.8) / 19.6) * 16383)
            target_pb = max(0, min(16383, mapped_val))
            if do_print: 
                print(f"[TILT] LEAD Mode | Tilt: {raw_tilt_y:.2f} (ACTIVE!) | PB: {target_pb}")
    else:
        target_pb = 8192
        # Print once if we are trying to bend but are in the wrong mode
        if do_print and raw_tilt_y > 3.0: 
            print(f"[TILT] Bending, but Mode is {current_mode} / State is {system_state}")

    if do_print:
        hw.last_print_time = now # Reset the print timer

    # Apply to SynthEngine and MIDI
    ...
    if synth.current_pitch_bend != target_pb:
        ...
        # Internal Audio Bend
        if hasattr(synth, 'pitch_bend'): synth.pitch_bend = target_pb
        elif hasattr(synth, 'set_pitch_bend'): synth.set_pitch_bend(target_pb)
        
        # External USB MIDI Bend
        if get_val("SYS", "MIDI OUT") == 1 and midi is not None:
    ...
    if g_val > 0.0:
        for tag, notes in synth.active_live.items():
            for n in notes:
                target = synth.glide_targets.get(n)
                if target and n.frequency != target:
                    diff = target - n.frequency
                    if abs(diff) < 0.5: n.frequency = target
                    else: n.frequency += diff * (0.01 / g_val)
    ...
    for btn_idx in just_pressed:
        ...
        if not in_step_mode and fn3_held and btn_idx == 6:
            ...
            continue
        if in_step_mode:
            if fn2_held and fn3_held and btn_idx == 6: 
                track_prefix = f"T{active_track}_"
